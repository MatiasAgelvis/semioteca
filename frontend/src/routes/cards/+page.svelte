<script lang="ts">
    ...
    let returnToCardId = $state<string | null>(
        typeof sessionStorage !== "undefined"
            ? sessionStorage.getItem("cards:returnTo")
            : null,
    );
    ...
    const displayCards = $derived(
        fullResultsMode ? rankedSearchResults : filteredCards,
    );
    ...
    async function scrollToCard(id: string) {
        ...
        let node =
            cardElements.get(id) ?? document.getElementById(`card-${id}`);
        if (!node) {
            await tick();
            node =
                cardElements.get(id) ?? document.getElementById(`card-${id}`);
    ...
    async function setupObserver() {
        ...
        if (
            ...
            (!focusedCardId ||
                !displayCards.some((c) => c.id === focusedCardId))
        ) {
            focusedCardId = displayCards[0].id;
        }
    }
    ...
    $effect(() => {
        ...
        if (
            !selectedBook ||
            !booksModel.some((book) => book.key === selectedBook)
        ) {
        ...
        >
            {#if fullResultsMode}
                ...
                <button
                    ...
                    type="button"
                    onclick={closeFullResultsMode}>Volver al modo libro</button
                >
            {:else}
        ...
        <div class="mt-6">
            ...
            <div class="drawer drawer-end lg:hidden">
                ...
                <div class="drawer-side z-40">
                    <label
                        for="cards-mobile-drawer"
                        class="drawer-overlay"
                        aria-label="Cerrar panel lateral"
                    ></label>
                    <div
                        class="min-h-full w-80 max-w-[85vw] space-y-4 bg-base-200 p-4"
                    >
                ...
                <div class="space-y-5">
                    ...
                    {:else}
                        ...
                        {#if displayCards.length === 0}
                            <p class="text-sm">
                                No hay tarjetas que coincidan con la búsqueda o
...
>
    ...
    >
        ...
        <div
            class="sm:max-h-[55vh] min-h-25 flex-1 space-y-3 overflow-y-auto px-6 py-5"
        >
            {#if !hasSearchCriteria}
                <p
                    class="rounded-2xl border border-dashed border-base-300 px-4 py-8 text-center text-sm opacity-70"
                >
                    Busca en autores, libros, páginas y contenido. Al elegir un
                    resultado, se abrirá su libro y se hará scroll a la tarjeta.
                </p>
