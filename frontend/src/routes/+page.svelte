...
<svelte:head>
  ...
  <meta
    name="description"
    content={`Fichero de ciencias del significado: tarjetas, ensayos y documentos sobre semiótica, pragmática, lingüística y filosofía del lenguaje escritos por ${SITE_AUTHOR}.`}
  />
</svelte:head>
...
<div class="mx-auto w-full max-w-7xl px-5 py-10 lg:px-10">
  ...
  >
    ...
    <p class="mt-6 max-w-2xl text-base leading-7 text-base-content/70">
      Un fichero textual de diversos libros sobre semántica, pragmática, lingüística, semiótica y
      filosofía del lenguaje elaborado con fines didácticos, para apoyar la formación de alumnos y
      tesistas. Además se incluyen diversos artículos y ensayos en estas materias, escritos por el
      profesor Valmore Agelvis como ejemplos de la aplicación de estos ficheros.
    </p>
    <div class="mt-8 flex gap-8">
      <div>
        <p class="text-3xl font-black tracking-tight text-base-content">{data.totalCards}</p>
        <p class="mt-1 text-xs font-semibold tracking-[0.2em] text-base-content/50 uppercase">
          Tarjetas
        </p>
      </div>
  ...
  >
    ...
    >
      ...
      <p class="mt-3 text-sm leading-7 text-base-content/70">
        Búsqueda, filtros por obra y detalle por ficha bibliográfica.
      </p>
    </a>
    <a
      class="group rounded-box border border-base-300/70 bg-base-100/80 p-5 transition hover:border-primary/30"
    ...
    >
      ...
      <p class="mt-3 text-sm leading-7 text-base-content/70">
        Análisis de cine y series desde las ciencias del significado.
      </p>
    </a>
    {#if SHOW_DOCS}
      <a
      ...
      >
        ...
        <p class="mt-3 text-sm leading-7 text-base-content/70">
          Acceso web y descarga directa de recursos PDF publicados.
        </p>
      </a>
    {/if}
    {#if SHOW_CV}
      ...
      >
        ...
        <p class="mt-3 text-sm leading-7 text-base-content/70">
          Resumen académico y enlaces a secciones curriculares.
        </p>
      </a>
    {/if}
  </section>
  ...
  {#if data.recentPosts.length > 0}
    <section class="mt-8">
      <h2 class="mb-4 text-xs font-semibold tracking-[0.2em] text-base-content/50 uppercase">
        Últimas entradas
      </h2>
