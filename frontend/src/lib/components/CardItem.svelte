<script lang="ts">
    ...
    const authorSegments = $derived(
        getHighlightSegments(card.author, searchTerms),
    );
    const bookSegments = $derived(getHighlightSegments(card.book, searchTerms));
    const pageSegments = $derived(
        getHighlightSegments(card.page ?? "s/p", searchTerms),
    );
    ...
    const compactText = $derived(
        searchActive
            ...
            : (() => {
                  const stripped = card.content.replace(
                      /\[\[IMAGE:\d+\]\]\n?/g,
                      "",
                  );
                  return stripped.length > 350
                      ? stripped.slice(0, 350).trimEnd() + "\u2026"
                      : stripped;
              })(),
    );
    const contentSegments = $derived(
        getHighlightSegments(compactText, searchTerms),
    );
    ...
    const visibleTags = $derived(
        card.tags?.filter((tag) => tag.trim().length > 0) ?? [],
    );
    ...
    const expandedParts = $derived.by<ContentPart[]>(() => {
        const imageMap = new Map(
            card.images.map((img) => [img.placeholder_id, img]),
        );
        ...
        for (let i = 0; i < chunks.length; i++) {
            if (i % 2 === 0) {
                if (chunks[i].trim())
                    parts.push({ kind: "text", text: chunks[i] });
            } else {
    ...
    async function copyCitation() {
        ...
        showToast(
            copied ? "Cita copiada" : "No se pudo copiar",
    ...
    async function copyCardText() {
        ...
        showToast(
            copied ? "Texto copiado" : "No se pudo copiar",
            copied ? "success" : "error",
        );
    }
        ...
        {#if expanded}
            <div class="mt-1 space-y-3">
                {#each expandedParts as part}
                    {#if part.kind === "text"}
                        <p
                            class="whitespace-pre-wrap text-sm leading-7 opacity-80"
                        >
                            <HighlightedText
            ...
            <div class="flex flex-wrap items-center justify-end gap-2">
                <details bind:this={detailsEl} class="dropdown dropdown-end">
                    ...
                    >
                        ...
                        <li>
                            <button type="button" onclick={copyCardText}
                                >Copiar texto</button
                            >
                        </li>
                    </ul>
