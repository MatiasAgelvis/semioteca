...
export function getRankedSearchResults(
  ...
  return cards
    ...
    .map(({ card, index, searchableText }) => ({
      ...
      score: (() => {
        const coverageBonus =
          terms.length > 0 ? (countMatchedTerms(searchableText, terms) / terms.length) * 20 : 0;
        const authorScore = searchFields.authorBook
          ? Math.min(getMatchCount(card.author, terms), 3) * 6
          : 0;
        const bookScore = searchFields.authorBook
          ? Math.min(getMatchCount(card.book, terms), 3) * 5
          : 0;
        const pageScore = searchFields.page
          ? Math.min(getMatchCount(card.page ?? '', terms), 3) * 4
          : 0;
        const tagScore =
          searchFields.tags && card.tags
            ? Math.min(
                card.tags.reduce((sum, t) => sum + getMatchCount(t, terms), 0),
                3,
              ) * 5
            : 0;
